...
impl<R: TenantRepository> TenantService<R> {
    ...
    pub fn create_tenant(&self, name: TenantName) -> Result<Tenant, ApplicationError> {
    ...
    pub fn delete_tenant(
        ...
        let active_business_count = business_repository.count_active_by_tenant(id)?;
...
mod tests {
    ...
    #[test]
    fn create_tenant_saves_and_returns_tenant() {
    ...
    #[test]
    fn rename_tenant_increments_version() {
        let repo = InMemoryTenantRepository::new();
        let service = TenantService::new(repo);
        let tenant = service
            .create_tenant(TenantName::new("Tenant A").unwrap())
    ...
    #[test]
    fn rename_tenant_rejects_stale_version() {
        let repo = InMemoryTenantRepository::new();
        let service = TenantService::new(repo);
        let tenant = service
            .create_tenant(TenantName::new("Tenant A").unwrap())
        ...
        let result = service.rename_tenant(
            tenant.id(),
            TenantName::new("Tenant A Baru").unwrap(),
            tenant.version() + 1,
        );
    ...
    #[test]
    fn rename_tenant_fails_when_not_found() {
        let repo = InMemoryTenantRepository::new();
        let service = TenantService::new(repo);

        let result = service.rename_tenant(TenantId::new(), TenantName::new("X").unwrap(), 0);

        assert!(matches!(result, Err(ApplicationError::TenantNotFound)));
    }

    #[test]
    fn delete_tenant_fails_when_active_business_exists() {
        ...
        business_repo.save(&business).unwrap();

        let result = service.delete_tenant(tenant.id(), &business_repo);
    ...
    #[test]
    fn delete_tenant_succeeds_when_no_active_business() {
        let tenant_repo = InMemoryTenantRepository::new();
        let business_repo = InMemoryBusinessRepository::new();
        let service = TenantService::new(tenant_repo);
        ...
        service.delete_tenant(tenant.id(), &business_repo).unwrap();

        let stored = service.repository.find_by_id(tenant.id()).unwrap().unwrap();
